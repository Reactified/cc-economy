...
--/ Rednet Setup /--
for _, side in ipairs({"left", "right", "back", "top", "bottom", "front"}) do
...
rednet.broadcast({
    isEconomy = true,
    request = "SERVER-HANDSHAKE"
})
for i=1,5 do
    local id,cmd = rednet.receive(0.1)
    if cmd == "ECONOMY-SERVER-HANDSHAKE-COMPLETE" then
        server = id
        break
    end
end
...
function register(username,password)
    ...
    local id,cmd = false,false
    while true do
        id,cmd = rednet.receive(0.1)
        if id == server then
            break
        end
    end
...
function verify(username,password)
    rednet.send(server,{
        ...
        request = "VERIFY",
        username = username,
        password = sha256(sha256(username..password)..textutils.formatTime(os.time())),
    })
    local id,cmd = false,false
    while true do
        id,cmd = rednet.receive(0.1)
...
function send(fromUsername,fromPassword,toUsername,amount)
    rednet.send(server,{
        isEconomy = true,
        request = "TRANSFER",
        fromUsername = fromUsername,
        authPassword = sha256(sha256(fromUsername..fromPassword)..textutils.formatTime(os.time())),
